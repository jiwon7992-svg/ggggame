...
(function () {
  ...
  // ---- 키 입력 안정화 ----
  // Streamlit의 components iframe 안에서는 포커스가 자주 바뀌어
  // keyup/keydown 한쪽이 유실될 수 있습니다. window/document 양쪽에서
  // 입력을 받고, 실제 게임 iframe에 포커스를 유지합니다.
  ...
  // keydown repeat은 무시하지 않고 상태만 유지합니다.
  // keyup 유실을 대비해 blur/visibilitychange에서 모든 키를 초기화합니다.
  ...
  // iframe 내부가 키보드 입력을 받을 수 있도록 캔버스에 포커스를 둡니다.
  canvas.tabIndex = 0;
  canvas.focus();
  canvas.addEventListener('pointerdown', () => canvas.focus());

  // 모바일 버튼도 누르고 있는 동안 부드럽게 이동합니다.
  function bindControlButton(button, dir) {
  ...
  window.crossyMove = function (dir) {
    ...
    // 버튼 클릭 1회는 정확히 한 번의 이동으로 처리합니다.
    // setTimeout + pointer 이벤트가 겹치며 입력이 씹히던 문제를 제거합니다.
    setKey(dir, true);
    setTimeout(() => setKey(dir, false), 90);
  };
  ...
  document.querySelectorAll('.ctrl-btn').forEach((button) => {
    ...
    const dir = text === '▲' ? 'up' :
                ...
                text === '◀' ? 'left' :
                text === '▶' ? 'right' : null;
    if (dir) bindControlButton(button, dir);
  });
...
<div style="display:flex; flex-direction:column; align-items:center; font-family: sans-serif;">
  <canvas id="game" tabindex="0" style="border-radius:12px; box-shadow:0 4px 14px rgba(0,0,0,0.25); touch-action:none; outline:none;"></canvas>
